#!/usr/bin/env python3
"""
Fetch the first image of every droid scene (lowest q-number per scene).

Reads data/index.jsonl to find the first entry per droid scene, then streams
the HuggingFace dataset and saves that image.

Output:
  data/droid_first/entries.jsonl        — one entry per scene + image_path
  data/droid_first/images/<id>.jpg      — image files

Usage:
  python scripts/fetch_droid_first.py
  python scripts/fetch_droid_first.py --split test
  python scripts/fetch_droid_first.py --hf-token hf_...

Resume: already-fetched IDs are skipped.
"""
...
def load_first_entries(index_path: Path, split: str) -> dict[str, dict]:
    """Return {entry_id: entry} for the lowest-q entry of each droid scene."""
...
def main():
    p = argparse.ArgumentParser(description="Fetch first image of every droid scene")
    ...
    print("Loading first entries per droid scene...", flush=True)
    by_id = load_first_entries(index_path, args.split)
    if not by_id:
        print(f"No droid entries found for split={args.split!r}")
        sys.exit(0)
    print(f"Found {len(by_id)} droid scenes")

    out_dir = project_root / "data" / "droid_first"
    ...
    if remaining:
        print(f"  WARNING: {len(remaining)} scene IDs not found in dataset")
    print(f"View with:\n  python scripts/view_scene.py {out_jsonl}")
